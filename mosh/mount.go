...
func (m *mount) auth(w http.ResponseWriter, r *http.Request) error {
	...
	if !m.sql.Authentication(m.database, pair[0], pair[1]) {
		http.Error(w, "wrong user or password", http.StatusUnauthorized)
		return err
	} else {
		m.logger.Log("User %s logged in", pair[0])
	}
...
func (m *mount) meta(w http.ResponseWriter, r *http.Request) {
	...
	songReader := strings.NewReader(string(song))
	...
	m.sql.InsertMetaData(m.database, song)
...
/*
write - Authenticate SOURCE and write stream from it to appropriate mount buffer
*/
func (m *mount) write(w http.ResponseWriter, r *http.Request) {
	if !m.server.checkSources() {
		m.logger.Error("Number of sources exceeded")
		http.Error(w, "Number of sources exceeded", http.StatusForbidden)
		return
	}
...
/*
read - Send stream from requested mount to client
*/
func (m *mount) read(w http.ResponseWriter, r *http.Request) {
	...
	if !m.server.checkListeners() {
		...
		http.Error(w, "Number of listeners exceeded", http.StatusForbidden)
		return
	}
